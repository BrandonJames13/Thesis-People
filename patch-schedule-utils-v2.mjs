...
const newReturn = `  // ── Generate subject resolution report for debugging ──────────────────────
...
if (content.includes(oldReturn) && !content.includes(newReturn)) {
  content = content.replace(oldReturn, newReturn);
  console.log("  [OK] Added report generation");
} else if (content.includes(newReturn)) {
  console.log("  [SKIP] Report generation already added");
} else {
  console.error("  [ERROR] Could not find return statement to modify");
  process.exit(1);
}
...
function generateSubjectResolutionReport(cache, conflictCount) {
  ...
  console.warn("Total conflicts created from unresolved subjects: " + conflictCount);
}
